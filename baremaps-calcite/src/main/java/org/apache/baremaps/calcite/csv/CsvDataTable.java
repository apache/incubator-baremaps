...
public class CsvDataTable implements DataTable {
  ...
  public CsvDataTable(String name, File csvFile, char separator, boolean hasHeader)
      throws IOException {
    this(name, csvFile, separator, hasHeader, inferColumns(csvFile, separator, hasHeader));
  }
  ...
  public CsvDataTable(String name, File csvFile, char separator, boolean hasHeader,
    ...
    this.dataSchema = new DataSchema(name, columns);
  }
    ...
    if (hasHeader) {
      ...
      try (MappingIterator<Map<String, String>> iterator = mapper.readerFor(Map.class)
        ...
        for (String colName : inferred.getColumnNames()) {
          columns.add(new DataColumnFixed(colName, Cardinality.OPTIONAL, Type.STRING));
        }
      }
    } else {
      ...
      try (MappingIterator<List<String>> iterator = mapper.readerFor(List.class)
        ...
        if (iterator.hasNext()) {
          ...
          for (int i = 0; i < numColumns; i++) {
            columns.add(
                new DataColumnFixed("column" + (i + 1), Cardinality.OPTIONAL, Type.STRING));
          }
        } else {
