 ...
 * @module
 */
abstract class AbstractDbase3ByteReader
    extends CommonByteReader<SQLInvalidDbaseFileFormatException, SQLDbaseFileNotFoundException>
    implements Dbase3ByteReader {
   ...
   */
  public AbstractDbase3ByteReader(File file)
      throws SQLDbaseFileNotFoundException, SQLInvalidDbaseFileFormatException {
  ...
  protected Charset toCharset(byte codePageBinaryValue) throws UnsupportedCharsetException {
    ...
    try {
      ...
      String message =
          format(
              Level.WARNING,
              "assert.wrong_charset_selection",
              dbfCodePage,
              getFile().getAbsolutePath());
