...
public class MappedByteReader extends AbstractDbase3ByteReader implements AutoCloseable {
    ...
    try {
      ...
      if (this.descriptorTerminator != 0x0D) {
        String message =
            format(
                Level.WARNING,
                "excp.filedescriptor_problem",
                getFile().getAbsolutePath(),
                "Character marking the end of the fields descriptors (0x0D) has not been found.");
  ...
  public int findColumn(String columnLabel, String sql) throws SQLNoSuchFieldException {
    ...
    if (columnLabel == null) {
      String message =
          format(
              Level.WARNING,
              "excp.no_such_column_in_resultset",
              columnLabel,
              sql,
              getFile().getName());
  ...
  private DBase3FieldDescriptor getField(int columnIndex, String sql)
      throws SQLIllegalColumnIndexException {
    if (columnIndex < 1 || columnIndex > getColumnCount()) {
      String message =
          format(Level.WARNING, "excp.illegal_column_index", columnIndex, getColumnCount());
