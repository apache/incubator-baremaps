...
public class DBFStatement extends AbstractStatement {
  ...
  public boolean execute(String sql) throws SQLException {
    ...
    } else throw unsupportedOperation("execute something else than a SELECT statement");
  }
  ...
  public void close() {
    ...
    // Check if all the underlying ResultSets that has been opened with this statement has been
    // closed.
    // If not, we log a warning to help the developer.
    if (this.openedResultSets.size() > 0) {
      log(
          Level.WARNING,
          "log.resultsets_left_opened",
          this.openedResultSets.size(),
          this.openedResultSets.stream()
              .map(DBFResultSet::toString)
  ...
  /**
   * Returns {@code true} if this statement has been closed or if the underlying connection is
   * closed.
   *
  ...
  public void assertNotClosed() throws SQLConnectionClosedException {
    ...
    if (this.isClosed) {
      throw new SQLConnectionClosedException(
          format(Level.WARNING, "excp.closed_statement", this.connection.getFile().getName()),
          null,
          this.connection.getFile());
    }
  }
  ...
  public void notifyCloseResultSet(DBFResultSet rs) {
    ...
    if (this.currentResultSet == rs) this.currentResultSet = null;

    if (this.openedResultSets.remove(rs) == false) {
      throw new RuntimeException(
          format(Level.SEVERE, "assert.resultset_not_opened_by_me", rs, toString()));
    }
  }
  ...
  public String toString() {
    return format(
        "toString",
        this.connection != null ? this.connection.toString() : null,
        isClosed() == false);
  }
}
