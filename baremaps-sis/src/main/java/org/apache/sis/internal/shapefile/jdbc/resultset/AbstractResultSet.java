...
/**
 * Holds all the unimplemented feature of a {@code ResultSet}. This is in order to avoid having a
 * ResultSet implementation of thousand lines and unreadable.
 *
 * <table class="sis">
 ...
 * </table>
 *
 * Furthermore, most methods expecting a column label of type {@code String} first invoke {@link
 * #findColumn(String)}, then invoke the method of the same name expecting a column index as an
 * {@code int}.
 *
...
public abstract class AbstractResultSet extends AbstractJDBC implements ResultSet {
   ...
   * Implementing this method will allow {@link #relative(int)} and other methods to work with their
  ...
  /**
   * Moves the cursor to the given row number (first row is 1). Special cases:
   *
   * <ul>
   *   <li>Negative numbers move to an absolute row position with respect to the end of the result
   *       set.
   *   <li>-1 moves on the last row.
   *   <li>0 moves the cursor before the first row.
   * </ul>
   *
   * This method is unsupported by default. Implementing this method will allow {@link
   * #relative(int)} and other methods to work with their default implementation.
   *
