...
public class CrudeSQLParser extends AbstractJDBC {
    ...
    public ConditionalClauseResolver parse() throws SQLInvalidStatementException {
        ...
        if (sql.toLowerCase().startsWith("select * from ") == false) {
            throw new SQLInvalidStatementException("Limited feature syntax", this.rs.getSQL(), this.rs.getFile());
        }
        ...
        if (whereCondition.isEmpty()) {
            throw new SQLInvalidStatementException("Where without conditions", this.rs.getSQL(), this.rs.getFile());
        }
        ...
        if (parts.length != 3) {
            throw new SQLInvalidStatementException("Limited feature conditional parsing", this.rs.getSQL(), this.rs.getFile());
        }
