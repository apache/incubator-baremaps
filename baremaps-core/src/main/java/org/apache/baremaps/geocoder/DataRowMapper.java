...
public class DataRowMapper implements Function<DataRow, Document> {
    ...
    public Document apply(DataRow dataRow) {
        ...
        for (int i = 0; i < columns.size(); i++) {
            ...
            try {
                ...
                logger.error("Error processing column '{}' with value '{}': {}", columnName, value, e.getMessage());
            }
        }
        return doc;
    }
    ...
    private Field[] createShapeFields(String fieldName, Geometry geometry) {
        if (geometry instanceof Point point) {
            ...
            logger.warn("Unsupported geometry type '{}' for field '{}'", geometry.getGeometryType(), fieldName);
            return new Field[0];
        }
    }
    ...
    private org.apache.lucene.geo.Polygon convertToLucenePolygon(org.locationtech.jts.geom.Polygon jtsPolygon) {
