...
-- XTRA LARGE
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_xl_filtered AS
SELECT
    id,
    tags -> 'landuse' as landuse,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_xl_clustered AS
SELECT
    landuse,
    geom,
    st_clusterdbscan(geom, 0, 0) OVER(PARTITION BY landuse) AS cluster
...
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_xl_grouped AS
SELECT
    landuse,
    cluster,
    st_collect(geom) AS geom
...
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_xl_buffered AS
SELECT
    landuse,
    st_buffer(geom, -78270 / power(2, 8), 'join=mitre') AS geom
FROM osm_landuse_xl_grouped;

CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_xl_exploded AS
SELECT
    landuse,
    (st_dump(geom)).geom AS geom
FROM osm_landuse_xl_buffered;

CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_xl AS
SELECT
    row_number() OVER () AS id,
    jsonb_build_object('landuse', landuse) AS tags,
    geom AS geom
...
-- LARGE
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_l_filtered AS
SELECT
    id,
    tags -> 'landuse' as landuse,
...
CREATE MATERIALIZED VIEW IF NOT EXISTSosm_landuse_l_clustered AS
SELECT
    landuse,
    geom,
    st_clusterdbscan(geom, 0, 0) OVER(PARTITION BY landuse) AS cluster
...
CREATE MATERIALIZED VIEW IF NOT EXISTSosm_landuse_l_grouped AS
SELECT
    landuse,
    cluster,
    st_collect(geom) AS geom
...
CREATE MATERIALIZED VIEW IF NOT EXISTSosm_landuse_l_buffered AS
SELECT
    landuse,
    st_buffer(geom, 0.5 * -78270 / power(2, 7), 'join=mitre') AS geom
FROM osm_landuse_l_grouped;

CREATE MATERIALIZED VIEW IF NOT EXISTSosm_landuse_l_exploded AS
SELECT
    landuse,
    (st_dump(geom)).geom AS geom
FROM osm_landuse_l_buffered;

CREATE MATERIALIZED VIEW IF NOT EXISTSosm_landuse_l AS
SELECT
            row_number() OVER () AS id,
            jsonb_build_object('landuse', landuse) AS tags,
            geom AS geom
...
-- MEDIUM
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_m_filtered AS
SELECT
    id,
    tags -> 'landuse' as landuse,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_m_clustered AS
SELECT
    landuse,
    geom,
    st_clusterdbscan(geom, 0, 0) OVER(PARTITION BY landuse) AS cluster
...
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_m_grouped AS
SELECT
    landuse,
    cluster,
    st_collect(geom) AS geom
...
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_m_buffered AS
SELECT
    landuse,
    st_buffer(geom, 0.1 * -78270 / power(2, 6), 'join=mitre') AS geom
FROM osm_landuse_m_grouped;

CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_m_exploded AS
SELECT
    landuse,
    (st_dump(geom)).geom AS geom
FROM osm_landuse_m_buffered;

CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_m AS
SELECT
            row_number() OVER () AS id,
            jsonb_build_object('landuse', landuse) AS tags,
            geom AS geom
...
-- SMALL
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_s_filtered AS
SELECT
    id,
    tags -> 'landuse' as landuse,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_s_clustered AS
SELECT
    landuse,
    geom,
    st_clusterdbscan(geom, 0, 0) OVER(PARTITION BY landuse) AS cluster
...
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_s_grouped AS
SELECT
    landuse,
    cluster,
    st_collect(geom) AS geom
...
CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_s_buffered AS
SELECT
    landuse,
    st_buffer(geom, 0, 'join=mitre') AS geom
FROM osm_landuse_s_grouped;

CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_s_exploded AS
SELECT
    landuse,
    (st_dump(geom)).geom AS geom
FROM osm_landuse_s_buffered;

CREATE MATERIALIZED VIEW IF NOT EXISTS osm_landuse_s AS
SELECT
            row_number() OVER () AS id,
            jsonb_build_object('landuse', landuse) AS tags,
            geom AS geom
