...
CREATE TABLE osm_ways_geom AS (
    SELECT w.id, w.version, w.uid, w.timestamp, w.changeset, w.tags, w.nodes, CASE WHEN st_isclosed(g.geom) AND ST_Length(g.geom) > 3 THEN st_makepolygon(g.geom) ELSE g.geom END AS geom
...
CREATE TABLE osm_relations_geom AS (
    ...
    FROM osm_relations r JOIN (
        SELECT r.id, CASE WHEN r.tags -> 'type' = 'multipolygon' THEN st_makevalid(st_polygonize(w.geom ORDER BY ordinality))
