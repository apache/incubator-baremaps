...
public class PBFUtilTest {
  ...
  public void stream() throws FileNotFoundException {
    assertTrue(PBFUtil.fileBlocks(OSM_PBF_DATA).count() == 10);
  }

  @Test
  public void isHeaderBlock() throws IOException {
    assertTrue(PBFUtil.fileBlocks(OSM_PBF_DATA).filter(PBFUtil::isHeaderBlock).count() == 1);
  }

  @Test
  public void isDataBlock() throws IOException {
    assertTrue(PBFUtil.fileBlocks(OSM_PBF_DATA).filter(PBFUtil::isDataBlock).count() == 9);
  }

  @Test
  public void toHeaderBlock() throws FileNotFoundException {
    assertTrue(
        PBFUtil.fileBlocks(OSM_PBF_DATA)
