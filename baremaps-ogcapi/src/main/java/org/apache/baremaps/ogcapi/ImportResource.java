...
public class ImportResource {
  ...
  public Response uploadData(@FormDataParam("file") InputStream fileInputStream,
    ...
    // .extent(new Extent().spatial(new ExtentSpatial().bbox(List.of(List.of(
    ...
    // // Insert features
    // var features = fc.features(); while (features.hasNext()) {
    // SimpleFeature feature = (SimpleFeature) features.next();
    // HashMap<String, String> properties = new HashMap<>();
    // feature.getProperties().stream()
    // .filter(property -> !property.getName().getLocalPart().equals("geometry"))
    // .forEach( property -> properties.put(property.getName().getLocalPart(),
    // property.getValue().toString()));
    // Object geom = feature.getDefaultGeometryProperty().getValue();
    // handle.createUpdate( String.format("insert into \"%s\" (tags, geom) values (:tags,
    // ST_Transform(ST_SetSRID(CAST(:geom as geometry), 4326), 3857))", collection.getId()))
    // .bind("tags", properties) .bind("geom", geom.toString()) .execute(); }
    // });
