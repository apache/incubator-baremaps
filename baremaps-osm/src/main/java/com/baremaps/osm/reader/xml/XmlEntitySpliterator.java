...
public class XmlEntitySpliterator implements Spliterator<Object> {

  public static final String HEADER = "osm";
  public static final String REPLICATION_TIMESTAMP = "osmosis_replication_timestamp";
  public static final String REPLICATION_SEQUENCE_NUMBER = "osmosis_replication_sequence_number";
  ...
  public XmlEntitySpliterator(XMLEventReader reader) {
    this.reader = reader;
  }
  ...
  private LocalDateTime readReplicationTimestamp(StartElement element) {
    Attribute attribute = element.getAttributeByName(QName.valueOf(REPLICATION_TIMESTAMP));
    if (attribute == null) {
      attribute = element.getAttributeByName(QName.valueOf(TIMESTAMP));
    }
    return attribute == null ? null : LocalDateTime.parse(attribute.getValue(), format);
  }
  ...
  private Node readNode(StartElement element)
    ...
    int version = readVersion(element);
    LocalDateTime timestamp = readTimestamp(element);
  ...
  private int readVersion(StartElement element) {
    Attribute attribute = element.getAttributeByName(QName.valueOf(VERSION));
    return attribute == null ? 0 : Integer.parseInt(attribute.getValue());
  }

  private LocalDateTime readTimestamp(StartElement element) {
    Attribute attribute = element.getAttributeByName(QName.valueOf(TIMESTAMP));
    return attribute == null ? null : LocalDateTime.parse(attribute.getValue(), format);
  }
    ...
    try {
      while (reader.hasNext()) {
        ...
        if (event.isStartElement()) {
          ...
          switch (name) {
            case HEADER:
              consumer.accept(readHeader(startElement));
              return true;
            case NODE:
              consumer.accept(readNode(startElement));
              return true;
            case WAY:
              consumer.accept(readWay(startElement));
              return true;
            case RELATION:
              consumer.accept(readRelation(startElement));
              return true;
            default:
              break;
          }
