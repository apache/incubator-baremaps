...
public class PostgisRelationStore implements Store<Relation> {
  ...
  public Relation get(Long id) throws StoreException {
    try (Connection connection = dataSource.getConnection();
      ...
      if (result.next()) {
        ...
        Integer[] types = (Integer[]) result.getArray(7).getArray();
  ...
  public void put(Relation entity) throws StoreException {
    try (Connection connection = dataSource.getConnection();
      ...
      Object[] types = entity.getMembers().stream().map(Member::getType).map(MemberType::getNumber).toArray();
      statement.setObject(8, connection.createArrayOf("int", types));
    ...
    try (Connection connection = dataSource.getConnection()) {
      ...
      try (CopyWriter writer = new CopyWriter(new PGCopyOutputStream(pgConnection, COPY))) {
        ...
        for (Relation entity : entities) {
          ...
          writer.writeIntegerList(entity.getMembers().stream().map(Member::getType).map(MemberType::getNumber).collect(Collectors.toList()));
